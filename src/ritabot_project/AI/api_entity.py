...
#gunicorn --bind 0.0.0.0:5000 wsgi_entity --certfile=/etc/nginx/ssl/domain.crt --keyfile=/etc/nginx/ssl/domain.key
...
countries_en =list(data_region_countries["Name_EN"].dropna())
countries_fr =list(data_region_countries["Name_FR"].dropna())
countries_ar =list(data_region_countries["Name_AR"].dropna())
...
def  entity_country(sentence, language):
    entitys =[]
    ...
    if language == 'arabic':
        countries = countries_ar
    if language == 'tunisian':
        countries = countries_en + countries_fr + countries_ar
    for country in countries:
        index = sentence.find(country.lower())
        if index != -1 :
            entitys.append({"start": index, "end": index + len(country), "text": country, "type": "COUNTRY", "value": country})
            break
    return entitys
...
def entity_URL(sentence):
    ...
    #
    # return entitys
    URL_REGEX = re.search("(?P<url>(https?://[^\s]+)|(www\.)[^\s]+)", sentence).group("url")
    index = sentence.find(URL_REGEX)
    entitys.append({"start": index, "end": index + len(URL_REGEX), "text": URL_REGEX, "type": "URL", "value": URL_REGEX})
...
def entity_zip_code(sentence):
    ...
    for zip_code in zip_codes:
        ...
        if index != -1:
            entitys.append({"start": index, "end": index + len(str(zip_code)), "text": str(zip_code), "type": "ZIPCODE", "value": str(zip_code)})
...
def get_entity():
    ...
    try:
        ...
        if (data["COUNTRY"]["value"]):
            entitys = entitys + entity_country(sentence, language)
        if (data["EMAIL"]["value"]):
